...
class Catalog_local extends Catalog
{
    ...
    private function _verify_chunk($tableName, $chunk, $chunk_size): int
    {
        ...
        $sql            = match ($tableName) {
            'album' => "SELECT `album`.`id`, MIN(`song`.`file`) AS `file`, MIN(`song`.`update_time`) AS `min_update_time` FROM `album` LEFT JOIN `song` ON `song`.`album` = `album`.`id` WHERE `album`.`catalog` = ? AND (`song`.`update_time` IS NULL OR song.update_time < " . $this->last_update . ") GROUP BY `album`.`id` ORDER BY MIN(`song`.`file`) DESC $count, $chunk_size",
